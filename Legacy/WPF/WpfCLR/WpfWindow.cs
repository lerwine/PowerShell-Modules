...
namespace WpfCLR
{
    ...
    {
        public const string XmlNamespaceURI_XmlNamespace = "http://www.w3.org/2000/xmlns/";
        ...
        private XmlDocument _windowXaml = null;

        public XmlDocument WindowXaml
        {
            get
            {
                XmlDocument windowXaml = _windowXaml;
                if (windowXaml == null)
                ...
                windowXaml.Attributes.Append(windowXaml.CreateAttribute("xmlns", "x", XmlNamespaceURI_XmlNamespace)).Value = XmlNamespaceURI_Xaml;
                windowXaml.Attributes.Append(windowXaml.CreateAttribute("xmlns", "d", XmlNamespaceURI_XmlNamespace)).Value = XmlNamespaceURI_Blend;
                windowXaml.Attributes.Append(windowXaml.CreateAttribute("xmlns", "mc", XmlNamespaceURI_XmlNamespace)).Value = XmlNamespaceURI_MarkupCompatibility;
            ...
            set
            {
                if (value != null)
                {
                    if (_windowXaml != null && ReferenceEquals(value, _windowXaml))
                        return;
                ...
                {
                    if (_windowXaml != null)
                        _windowXaml.NodeRemoving -= WindowXaml_NodeRemoving;
                    return;
                }

                if (value.DocumentElement.NamespaceURI != XmlNamespaceURI_Presentation || value.DocumentElement.LocalName != XmlElementName_Window)
        ...
        {
            if (ReferenceEquals(e.Node, _windowXaml.DocumentElement))
                throw new InvalidOperationException("Cannot remove root node.");
        }

        public int Width
        {
            get { return GetAttributeAsInt32(WindowXaml.DocumentElement, XmlAttributeName_Width, 0); }
            set
            {
                if (value < 1)
                    throw new ArgumentOutOfRangeException();

                SetAttributeValue(WindowXaml.DocumentElement, XmlAttributeName_Width, value);
            }
        }

        public int Height
        {
            get { return GetAttributeAsInt32(WindowXaml.DocumentElement, XmlAttributeName_Height, 0); }
            set
            {
                if (value < 1)
                    throw new ArgumentOutOfRangeException();

                SetAttributeValue(WindowXaml.DocumentElement, XmlAttributeName_Height, value);
            }
        }
