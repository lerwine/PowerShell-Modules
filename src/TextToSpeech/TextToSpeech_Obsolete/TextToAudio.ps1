...
Function Get-NormalizedLines {
    ...
    Process {
        'Source Text:' | Write-Host -ForegroundColor Green;
        $InputText | Write-Host;
        ...
        if ($NormalizedLine.Length -eq 0) {
            ...
            if ($m.Success -and $CurrentLine.Length -gt 0) {
                if ($CurrentLine.Length -gt 0) {
                    $CurrentLine | Write-Output;
                    'Output Line:' | Write-Host -ForegroundColor Green;
                    $CurrentLine | Write-Host;
                }
...
Add-ChoiceDescription -ChoiceCollection $choices -Label 'Save' -HelpMessage 'Save to file only';
...
while ($selectedChoice -ne $null) {
    ...
    switch ($selectedChoice) {
        'Speak' {
            ...
            Write-Progress -Activity "Text to Speech" -Status $voice.Status.RunningState -PercentComplete 0 -CurrentOperation "Starting"
            foreach ($line in $content) {
                ...
                do {
                    ...
                    if ($c -ne $lastPercentage) {
                        $lastPercentage = $c;
                        Write-Progress -Activity "Text to Speech" -Status $status.RunningState -PercentComplete $lastPercentage -CurrentOperation "Speaking";
                    }
            ...
            Write-Progress -Activity "Saving to file" -Status $voice.Status.RunningState -PercentComplete 99 -CurrentOperation "Saving" -Completed;
            ...
            $SaveFileDialog.Filter = 'Wave Sound Files (*.wav)|*.wav|All Files (*.*)|*.*';
            $SaveFileDialog.FilterIndex = 0;
            if ([String]::IsNullOrEmpty($AudioDirectory)) {
                $SaveFileDialog.InitialDirectory = [System.Environment]::GetFolderPath([System.Environment+SpecialFolder]::MyMusic);
            } else {
                $SaveFileDialog.InitialDirectory = $AudioDirectory;
            }
            ...
            if ($DialogResult -ne $null -and $DialogResult -eq [System.Windows.Forms.DialogResult]::OK) {
                $AudioDirectory = $SaveFileDialog.FileName | Split-Path -Parent;
                if ([System.IO.File]::Exists($SaveFileDialog.FileName)) { [System.IO.File]::Delete($SaveFileDialog.FileName) }
                $SpeechSynthesizer = New-Object -TypeName 'System.Speech.Synthesis.SpeechSynthesizer';
                try {
                    $SpeechAudioFormatInfo = New-Object -TypeName 'System.Speech.AudioFormat.SpeechAudioFormatInfo' -ArgumentList (32000, [System.Speech.AudioFormat.AudioBitsPerSample]::Sixteen, [System.Speech.AudioFormat.AudioChannel]::Mono);
                    $SpeechSynthesizer.SetOutputToWaveFile($SaveFileDialog.FileName, $SpeechAudioFormatInfo);
                    foreach ($line in $content) { $SpeechSynthesizer.Speak($line) }
                } catch {
                    throw;
                } finally {
            ...
            Write-Progress -Activity "Text to Speech" -Status $voice.Status.RunningState -PercentComplete 100 -CurrentOperation "Finished" -Completed;
        ...
        'Save' {
            ...
            if ($queue.Count -eq 0) {
                $voice.Speak($line, 1) | Out-Null
                'Nothing to speak.' | Write-Warning;
            } else {
                $content = $queue[0].Text + "`r`n";
                for ($i = 1; $i -lt $queue.Count; $i++) {
                    $content = $content + "`r`n";
