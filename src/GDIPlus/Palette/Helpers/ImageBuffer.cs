...
using Erwine.Leonard.T.GDIPlus.Palette.Ditherers;
...
#pragma warning restore IDE0130 // Namespace does not match folder structure
{
    public class ImageBuffer : IDisposable
    {
        ...
        /// </value>
        public bool CanRead => lockMode == ImageLockMode.ReadOnly || lockMode == ImageLockMode.ReadWrite;
        ...
        /// </value>
        public bool CanWrite => lockMode == ImageLockMode.WriteOnly || lockMode == ImageLockMode.ReadWrite;
        ...
        public int GetBitOffset(int x) => fastBitX[x];
        ...
        public static Color GetColorFromPixel(Pixel pixel)
        {
            Color result;
        ...
        public static void SetColorToPixel(Pixel pixel, Color color, IColorQuantizer quantizer)
        {
            // determines whether the format is indexed
            if (pixel.IsIndexed)
            {
        ...
        private void TransformPerPixelBase(ImageBuffer target, IList<Point> path, Delegate transformAction, int parallelTaskCount = 4)
        {
            ...
            if (Width != target.Width || Height != target.Height)
                throw new ArgumentOutOfRangeException(nameof(target), "Both images have to have the same dimensions.");
            ...
            void transformPerPixel(LineTask lineTask)
            {
                ...
                Pixel sourcePixel = new(this);
                Pixel targetPixel = new(target);
        ...
        public static void ScanImageColors(Image sourceImage, IColorQuantizer quantizer, int parallelTaskCount = 4)
        {
            ...
            using ImageBuffer source = new(sourceImage, ImageLockMode.ReadOnly);
        ...
        public static Image QuantizeImage(ImageBuffer source, IColorQuantizer quantizer, IColorDitherer ditherer, int colorCount, int parallelTaskCount = 4)
        {
            ...
            using ImageBuffer target = new(result, lockMode);
        ...
        public static Image QuantizeImage(Image sourceImage, IColorQuantizer quantizer, IColorDitherer ditherer, int colorCount, int parallelTaskCount = 4)
        {
            ...
            // wraps source image to a buffer
            using ImageBuffer source = new(sourceImage, lockMode);
        ...
        public static double CalculateImageMeanError(ImageBuffer source, Image targetImage, int parallelTaskCount = 4)
        {
            ...
            // wraps source image to a buffer
            using ImageBuffer target = new(targetImage, ImageLockMode.ReadOnly);
            // use other override to calculate error
            return source.CalculateMeanError(target, parallelTaskCount);
        }
        ...
        public double CalculateNormalizedMeanError(ImageBuffer target, int parallelTaskCount = 4) => CalculateMeanError(target, parallelTaskCount) / 255.0;
        ...
        public static void ChangeFormat(ImageBuffer source, PixelFormat targetFormat, IColorQuantizer quantizer, out Image targetImage, int parallelTaskCount = 4)
        {
            ...
            // wraps target image to a buffer
            using ImageBuffer target = new(targetImage, ImageLockMode.WriteOnly);
        ...
        public Color GetPaletteColor(int paletteIndex) => cachedPalette[paletteIndex];
        ...
        /// </summary>
        private class LineTask(int startOffset, int endOffset)
        {
            /// <summary>
