...
namespace UnitTests
{
    ...
    public class IOUtilityUnitTest
    {
        ...
        public void PSInvocationBuilderTestMethod()
        {
            ...
            target.SynchronizedData["Five"] = 23;
            Assert.IsNull(target.InitialSessionState);
            Assert.IsNull(target.Input);
            Assert.IsNull(target.Settings);
            target.AddScript("$MyVar | Write-Output;");
            target.AddStatement();
            target.AddScript("$SynchronizedData.Five = 10; $MyVar = 5; $MyVar | Write-Output;");
            ...
            obj = result.Output[0].BaseObject;
            Assert.IsNotNull(obj);
            Assert.IsTrue(obj is int);
            Assert.AreEqual<int>(23, (int)obj);
            obj = result.Output[1].BaseObject;
            Assert.IsNotNull(obj);
            Assert.IsTrue(obj is int);
            Assert.AreEqual<int>(5, (int)obj);
            Assert.IsTrue(result.SynchronizedData.ContainsKey("Five"));
            obj = result.SynchronizedData["Five"];
            Assert.IsNotNull(obj);
            Assert.IsTrue(obj is int);
            Assert.AreEqual<int>(10, (int)obj);
            Assert.AreEqual(0, result.Errors.Count);
            Assert.AreEqual(0, result.Warnings.Count);
        }

        [TestMethod]
        public void PSInvocationBuilderAsyncTestMethod()
        {
            PSInvocationBuilder target = new PSInvocationBuilder();
            Assert.AreEqual<ApartmentState>(ApartmentState.STA, target.ApartmentState);
            Assert.AreEqual<PSThreadOptions>(PSThreadOptions.ReuseThread, target.ThreadOptions);
            ...
            Assert.IsNull(target.Input);
            Assert.IsNull(target.Settings);
            target.AddScript("$MyVar | Write-Output; $SynchronizedData.Five = 10; $MyVar = 5; $MyVar | Write-Output;");
            target.Variables.Add("MyVar", 23);
            PSInvocationResult result;
            PSAsyncInvocationResult asyncResult = target.GetResultAsync();
